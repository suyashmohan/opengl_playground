...
void draw(GLuint shaderProgram, GLuint texture1, GLuint texture2, Mesh mesh,
          Camera c, float *m);
...
int main(void) {
  ...
  glUniform1f(glGetUniformLocation(mat.shader, "material.shininess"),
              shininess);
  ...
  glUniform3fv(glGetUniformLocation(mat.shader, "light.ambient"), 1,
               (const GLfloat *)&la);
  glUniform3fv(glGetUniformLocation(mat.shader, "light.diffuse"), 1,
               (const GLfloat *)&ld);
  glUniform3fv(glGetUniformLocation(mat.shader, "light.specular"), 1,
               (const GLfloat *)&ls);
  glUniform3fv(glGetUniformLocation(mat.shader, "light.direction"), 1,
               (const GLfloat *)&ldir);
...
void processInput(GLFWwindow *window, Camera *c, float *r) {
  float angle = 1.0f;
